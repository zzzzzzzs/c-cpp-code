/**
 * @file
 * @brief This file contains malloc_dbg, calloc_dbg, free_dbg and printLeaks
 * implementations.
...
/* We must undef these macros in order to use the real malloc / calloc and free
 * functions */
#undef malloc
#undef calloc
#undef free
...
typedef struct MEMORY_INFORMATION {
  ...
  const char*
      functionName;  ///< Function in which malloc or calloc has been called
  size_t bytes;      ///< Number of bytes allocated
  int line;  ///< Line number (in file) corresponding to the malloc / calloc
             ///< call
...
/** Another global variable. This one is used to know if we already call the
 * atexit function.
 * */
int atexitCalled = 0;
 ...
 * @param memoryInfo Pointer to the doubly linked list used to store all of the
 * allocations
 ...
 */
mem_info* addMemInfo(mem_info* memoryInfo, void* ptrToReturn, size_t bytes,
  ...
  if (memoryInformation) memoryInformation->previous = newMemInfo;

  return newMemInfo;
}

/**
 * @brief inList function is used to know if an element is already in the
 * memoryInfo list.
 * @details This function is used to know if an allocation in a specific file at
 * a specific line already exists in the list.
 ...
 * @returns Position of the element in the list if the element is found, -1
 * otherwise.
 */
 ...
 * @details This function is used to edit the number of bytes allocated at a
 * specific line.
 ...
 * @details This function calls malloc and allocates the number of bytes passed
 * in the parameters. If the allocation succeeds then it add the pointer
 * returned by malloc in the mem_info list.
 * @param bytes Size of the allocation in bytes
 ...
 */
void* malloc_dbg(size_t bytes, int line, const char* filename,
  ...
  if (pos == -1) {
    ...
    memoryInformation = addMemInfo(memoryInformation, ptrToReturn, bytes, line,
 ...
 * @details This function calls calloc and allocates the number of bytes passed
 * in the parameters. If the allocation succeeds then it add the pointer
 * returned by malloc in the mem_info list.
 ...
 */
void* calloc_dbg(size_t elementCount, size_t elementSize, int line,
  ...
  // Add a new element in the mem_info list
  memoryInformation =
      addMemInfo(memoryInformation, ptrToReturn, elementCount * elementSize,
 ...
 * @details This function free the memory pointed by the pointer passed in
 * parameter. To free this pointer, we loop through the mem_info list and check
 * if we find the pointer. Once it's found, the pointer is freed and the element
...
void free_dbg(void* ptrToFree) {
  ...
  // We can loop through the list without any problems, the head is not the
  // pointer
  while (tmp) {
 ...
 * @details This function is called when the program exits. It loop through the
 * mem_info list and if it's not empty, it prints the memory leaks.
 * @returns Nothing.
 */
void printLeaks() {
  mem_info* tmp = memoryInformation;
