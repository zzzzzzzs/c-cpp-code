...
/** this value recommand to be 2^n , the larger it is, the less conflict map
 * will cause **/
...
#define map_for_each(pmap, entry, _iii_)                     \
  for ((_iii_) = 0; (_iii_) < (pmap)->table_size; ++(_iii_)) \
    for ((entry) = (pmap)->table[_iii_]; (entry) != NULL;    \
         (entry) = (entry)->next)

// assess the no conflict, we recommand you to call this when map size equals
// spar, if you use map_init_custom(....) to init a map.
...
void map_init_custom(map *map, size_t spar,
                     unsigned int (*hashfunc)(char *, int));
